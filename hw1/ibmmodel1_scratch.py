...
def initialize_translation11(source, target):
	...
	for k in range(len( source)):
		...
		for sword in source_sentence:
            ...
            for tword in target_sentence:
                if tword not in translation[sword]:
                    translation[tword][sword]=0.0
				translation[sword][tword] += 1.0
	for sword in translation:
		length = sum([translation[sword][tword] for tword in translation[sword][tword]])
		for tword in translation[sword]:
			translation[sword][tword] =float(translation[sword][tword]/ float(length)
	return translation
'''
def initialize_translation2(source,target):
    translation={}
    denom={}
    for k in range(len(source)):
        source_sentence=source[k]
    ...
    for sword in translation:
        for tword in translation[sword]:
            translation[sword][tword]=1.0/float(denom[tword])
    return translation
def initialize_translation3(source,target):
    translation={}
    denom={}
    for k in range(len(source)):
        source_sentence=source[k]
    ...
    for sword in translation:
        for tword in translation[sword]:
            translation[sword][tword]=float(translation[sword][tword])/float(denom[tword])
    return translation
def initialize_translation2(source,target):
    translation={}
    denom={}
...
def alignment_model1(source, target, translation):
	l=len(source)
	f=open('new3.a1i3.small.output.txt','w')
	for k in range(l):
		source_sentence = source[k]
		target_sentence = target[k]
		print_string=''
...
if __name__ == "__main__":
    ...
    target=[]
    p=Preprocessing()
    #with codecs.open('test.data','r','utf-8') as g:
    ...
    #t = initialize_translation2(source,target)
    while it < 5:
